...
7ב. \`alsoFits\` — שלושה ניחושים **הגיוניים ושגויים**: מה שילד באמת
   יכתוב אחרי הרמז הראשון. לא נרדפים של התשובה, ולא משהו שמכיל
   את התשובה בתוכו ("עוף בגריל" פסול ל"עוף"). המשחק משתמש בהם
   כדי להודות לילד שהוא חשב נכון, ולהציע רמז שיבדיל.
