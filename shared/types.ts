...
/**
 * ניחוש הגיוני שאינו התשובה.
 *
 * מחרוזת פשוטה = מתאים לכל הרמזים של החידה.
 * אובייקט = מתאים עד לרמז מסוים, ומשם ואילך הוא כבר נפסל —
 * וצריך להסביר בקצרה למה.
 */
export type AlsoFits =
  | string
  | {
      guess: string;
      /**
       * מספר הרמז (מ-1) שפוסל את הניחוש.
       *
       * "קרטיב" מתאים מצוין ל"אני קר מאוד וגר במקפיא", ונפסל
       * ברמז "אוכלים אותי בגביע". אחרי שהרמז הזה נחשף, לומר
       * "הניחוש מתאים לרמזים" זה פשוט לא נכון.
       */
      ruledOutBy: number;
      /** ההבדל, במשפט אחד — מה שנחשף כבר לא מסתדר איתו */
      because: string;
    };
